...
You are auditing this repository to produce three deliverables, organized around the ASDLC distinction between **agent constitution**, **architectural orientation**, and **behavioral specs**:
...
2. **Architectural orientation** under `docs/agents/` — human reference files for onboarding and deep dives. These are **not agent context** — they exist for humans and for agents to read on demand when they need detail on a specific area. **Skip files entirely** for areas that do not apply.
3. **Behavioral specs** under `docs/specs/` — produced by the **Spec Reversing** pattern. An inventory of features plus per-feature Blueprint+Contract specs that describe *what* the system does at the behavior level (not how the code is structured). Generated by delegating to `prompts/spec-inventory.md` and `prompts/spec-drafts.md` in Phase 5.

**Design principle:** Only put content in `AGENTS.md` that an agent cannot discover by reading the repo itself. Facts the agent can derive (tech stack from `package.json`, project structure from the filesystem, linter rules from config files) belong in `docs/agents/` as human reference — not in the agent constitution. Behavioral contracts of features belong in `docs/specs/`, not `docs/agents/` or `AGENTS.md`. Research shows that unnecessary instructions in context files actively harm agent performance by broadening exploration and increasing reasoning cost (Gloaguen et al., 2026).

If `docs/agents/` or `docs/specs/` already contains files from a previous audit, this is an **incremental update**. Read every existing file first, preserve what is still accurate, update what has changed, and remove what no longer applies. Note meaningful changes at the top of each updated file in a `## Changelog` section (date + one-liner). Keep only the **last 5 entries** — remove older ones to prevent unbounded growth.
...
Detailed documentation lives in `docs/agents/` (architectural orientation) and `docs/specs/` (behavioral contracts). Read on demand, not preloaded.

| Area | File | Read when… |
|------|------|------------|
| Overview | `docs/agents/overview.md` | Orientation, architecture, cross-repo dependencies |
| <area> | `docs/agents/<file>.md` | <when to read> |
| Feature inventory | `docs/specs/INVENTORY.md` | You need to know which features exist and which have specs |
| Per-feature specs | `docs/specs/<area>/<feature>.md` | About to modify a feature — read its Blueprint + Contract first |
```
...
### Phase 4: Reverse Behavioral Specs (Spec Reversing)

After `AGENTS.md` and `docs/agents/` are in place, produce behavioral specs for the repo's features using the ASDLC **Spec Reversing** pattern. This phase has two sub-runs:

1. **Inventory** — delegate to `prompts/spec-inventory.md`. It produces `docs/specs/INVENTORY.md`: a feature inventory, prioritized spec backlog, drift questions for human review, and the proposed spec file naming convention. Read that prompt and execute it in full.

2. **Drafts** — delegate to `prompts/spec-drafts.md`. It produces one Blueprint+Contract spec per feature under `docs/specs/<area>/<feature>.md`, using the inventory as the source of truth. Read that prompt and execute it in full.

> **Stop point for human review.** After the inventory is written, **pause and let a human review the Drift / Questions section** before running the drafts sub-run. Spec Reversing canonizes bugs as features unless a human filters them — the inventory is where that filtering happens.

> **Context limits.** For large repos, run the inventory and drafts in separate sessions to avoid context exhaustion. The inventory must exist on disk before the drafts run is started.

> **Cross-repo reconciliation.** Both sub-prompts read the workspace `AGENTS.md` at `../../AGENTS.md` to discover sibling repos and reconcile shared contracts. Make sure cross-repo dependencies are captured there (or in this repo's `docs/agents/overview.md`) before running this phase.

If specs are not needed yet (e.g. infrastructure-only repos with no behavioral features, or first-pass audits where only orientation is required), it is acceptable to skip Phase 4 and run it later. Note the skip and the reason in `docs/agents/overview.md` under **Known gaps & risks**.
...
1. **Path check** — every file path, directory, and link referenced in `docs/agents/`, `docs/specs/`, and `AGENTS.md` must exist in the repo. Remove or flag any that don't.
...
6. **Spec traceability check** (if Phase 4 ran) — every Blueprint claim and every Gherkin scenario in `docs/specs/` cites a `file_path:line` that exists. Any unverified scenario is flagged `[unverified]` and listed in the spec's **Open Questions**.
...
- **Minimal AGENTS.md.** The constitution carries only judgment-layer content. Everything else goes in `docs/agents/` (orientation) or `docs/specs/` (behavior).
...
- **Spec-anchored, not spec-as-source.** `docs/specs/` describes intent and contracts at the behavior level; it does not duplicate the code.
...
- **Flag unknowns.** If you can't determine something, say so — in `docs/agents/overview.md` for orientation gaps, in a spec's **Open Questions** for feature-level gaps.
- **Preserve existing docs.** If files already exist under `docs/agents/`, `docs/specs/`, or `AGENTS.md`, read first and improve rather than replacing blindly.
